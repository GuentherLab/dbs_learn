...
% timecourse colors
op.dbs_on_off_colors_tc = [0 0.2 0;... % train ON
                        0 0 1;...]; % train OFF
                        1 0.2 0.2]; % novel - only for Test phase
% bar colors
op.dbs_on_off_colors_bar = [0 0.5 0;... % train ON
                        0 0 1;...]; % train OFF
                        1 0.2 0.2]; % novel - only for Test phase
...
pwran_train.stimresp_fhlh_onvsoff = 'both'; 
...
plotcolors = table(op.dbs_on_off_colors_tc,op.dbs_on_off_colors_bar,'VariableNames',{'colors_tc','colors_bar'},'RowNames',{'on','off','novel'});
...
%% test phase acc bargraph
  if op.normalize_by_pretest
    ...
    hax = gca;      
    hbar = bar(con.test.acc_early_mean(test_rows_to_plot,:),'FaceColor','flat','CData',plotcolors.colors_bar(1:ncons,:));
    hold on
...
%% test phase - average over subjects
    ...
    for icon = 1:ncons
        ...
        thiscon = conlist{icon}; 
        hplot = plot(xtime, smoothdata(tc_mean, 2, op.smooth_method, op.smooth_windowsize), 'Color',op.dbs_on_off_colors_tc(icon,:));
        hplot.LineWidth = 2; 
            ...
            hold on
            hfill = fill([xtime, fliplr(xtime)], [tc_sem_lims_to_plot(1,:), fliplr(tc_sem_lims_to_plot(2,:))], plotcolors{thiscon,'colors_tc'},...
    ...
    hleg = legend(cellfun(@(x)['trained-',upper(x)],conlist,'UniformOutput',0));
            hleg.Position(2) = op.leg_height + 0.41; % adjust height up for tiledlayout 
            hleg.Position(1) = hleg.Position(1) - .115; % fine tune for figure size/shape
    title(['Test phase'])
   ...
   resptab = resp(:,{'sub','chan'});
    ...
    for ihalf = 1:2
        ...
        for ihalf = 1:2
            ...
            hfill = fill([xtime, fliplr(xtime)], [tc_sem_lims_plot(1,:), fliplr(tc_sem_lims_plot(2,:))], plotcolors{dbs_state,'colors_tc'},...
                'FaceAlpha',op.ebar_facealpha, 'LineStyle',op.ebar_line_style, 'HandleVisibility','off'); % standard error
    %         hfill.LineStyle = 'none'; % no border
            hfill.EdgeColor = op.ebar_edgecolor; 
        ...
        hplot = plot(xtime,mean_firsthalf_plot,'Color',plotcolors{dbs_state,'colors_tc'},'LineWidth',2,'LineStyle',op.linestyles_fhlh{1});
        hplot2 = plot(xtime,mean_lastthalf_plot,'Color',plotcolors{dbs_state,'colors_tc'},'LineWidth',2,'LineStyle',op.linestyles_fhlh{2});
...
nexttile(tilenum(htl,1,3))
    ...
    hbar = bar(onoff_test.stimresp_mean,'FaceColor','flat','CData',plotcolors.colors_bar(1:ncons,:));
    hold on
...
nexttile(tilenum(htl,2,3))
    ...
    hbar = bar(-onoff_train.mean_stimresp_fh_minus_lh,'FaceColor','flat','CData',plotcolors.colors_bar(1:ncons,:));
    hold on
...
[~, pwran_test.stimresp_onvsoff_p] = ttest(onoff_test{'off','stimresp'}{1}, onoff_test{'on','stimresp'}{1}, 'Tail',pwran_test.stimresp_onvsoff_tail);
