...
% previous version colors training timecourses according dbs ON vs. OFF; this version colors according to early vs. late trials
...
% pwran_train.stimresp_fhlh_onvsoff_tail = 'left'; 
pwran_train.stimresp_fhlh_onvsoff_tail = 'both'; 
...
[~,pwran_train.stimresp_on_early_vs_late_p] = ttest(train_on_stim_fh,train_on_stim_lh,'Tail','right');
    pwran_train.stimresp_on_early_vs_late_nsubs = sampsizepwr('t', [mean(train_on_stim_lh), std(train_on_stim_fh_minus_lh)], mean(train_on_stim_fh_minus_lh), 0.8, [], 'Tail','right') / op.chans_per_sub;
    pwran_train.stimresp_on_early_vs_late_power = sampsizepwr('t', [mean(train_on_stim_lh), std(train_on_stim_fh_minus_lh)], mean(train_on_stim_fh_minus_lh), [],op.planned_nchans, 'Tail','right');
pwran_train.stimresp_fhlh_onvsoff_power = sampsizepwr('t', [onoff_train{'on','mean_stimresp_fh_minus_lh'}, onoff_train{'on','std_stimresp_fh_minus_lh'}], ...
        onoff_train{'off','mean_stimresp_fh_minus_lh'}, [], op.planned_nchans, 'Tail',pwran_train.stimresp_fhlh_onvsoff_tail);
    pwran_train.stimresp_fhlh_onvsoff_nsubs = sampsizepwr('t', [onoff_train{'on','mean_stimresp_fh_minus_lh'}, onoff_train{'on','std_stimresp_fh_minus_lh'}], ...
        onoff_train{'off','mean_stimresp_fh_minus_lh'}, 0.8, [], 'Tail',pwran_train.stimresp_fhlh_onvsoff_tail) / op.chans_per_sub;
[~, pwran_train.stimresp_fhlh_onvsoff_p] = ttest(onoff_train.stimresp_fh_minus_lh{2},onoff_train.stimresp_fh_minus_lh{1}, 'Tail',pwran_train.stimresp_fhlh_onvsoff_tail ); 
