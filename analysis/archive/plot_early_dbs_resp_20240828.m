...
%%%% best quantitative result is with stimnormlate, divide_by off

op.divide_by_trial_baseline  = 0; 
op.divide_by_chan_baseline = 0;

op.stimnormlate = 1; % unclear what difference is between this and divide by trial baseline
   ...
   resptab = resp(:,{'sub','chan'});
    ...
    %         hfill.LineStyle = 'none'; % no border
        ...
        gobeep_lh = -mean(fhlh{'lh','resp'}{1}.rt,1);
         ...
         xline_stimoff(1) = xline(stimoff_to_sp_fh,'k','LineStyle',linestyles_first_last{1},'HandleVisibility','off');
         xline_stimoff(2) = xline(stimoff_to_sp_lh,'k','LineStyle',linestyles_first_last{2},'HandleVisibility','off');

          xline_stimon(1) = xline(stimon_to_sp_fh,'k','LineStyle',linestyles_first_last{1},'HandleVisibility','off');
         xline_stimon(2) = xline(stimon_to_sp_lh,'k','LineStyle',linestyles_first_last{2},'HandleVisibility','off');

          xline_gobeep_on(2) = xline(gobeep_fh,'g','LineStyle',linestyles_first_last{1},'HandleVisibility','off');
         xline_gobeep_on(2) = xline(gobeep_lh,'g','LineStyle',linestyles_first_last{2},'HandleVisibility','off');

         xline_sp_on = xline(0,'LineStyle','-','Color',[0.5 0.5 0.5],'HandleVisibility','off');
        
         % get speech off time
        if strcmp(dbs_state,'on')
            xline_sp_off_fh = xline(op.sp_dur_aprx_train_fh_on, 'LineStyle',linestyles_first_last{1},'Color',[1 0.1 0.1],'HandleVisibility','off');
            xline_sp_off_lh = xline(op.sp_dur_aprx_train_lh_on, 'LineStyle',linestyles_first_last{2},'Color',op.sp_off_color,'HandleVisibility','off');
            annot_speech_xloc = mean([op.sp_dur_aprx_train_fh_on, op.sp_dur_aprx_train_lh_on]) / 2; 
        elseif strcmp(dbs_state,'off')
            xline_sp_off_fh = xline(op.sp_dur_aprx_train_fh_off, 'LineStyle',linestyles_first_last{1},'Color',[1 0.1 0.1],'HandleVisibility','off');
            xline_sp_off_lh = xline(op.sp_dur_aprx_train_lh_off, 'LineStyle',linestyles_first_last{2},'Color',op.sp_off_color,'HandleVisibility','off');
