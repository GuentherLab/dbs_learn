function figTC=taskControlGUI()
    % Define global variables
    global pause_requested pause_isActive task_isRunning
    global taskState

    % Create an instance of PauseState
    if isempty(taskState)
        taskState = TaskState();
        if exist('pause_requested','var') && ~isempty(pause_requested)
            taskState.pause_requested = pause_requested;
        end
        if exist('pause_isActive','var') && ~isempty(pause_isActive)
            taskState.pause_requested = pause_isActive;
        end
        if exist('task_isRunning','var') && ~isempty(task_isRunning)
            taskState.task_isRunning = task_isRunning;
        end
    else
    ...
    figTC = figure('Name', 'Task Control', 'NumberTitle', 'off', ...
                 'Position', [100, 100, 400, 400], 'Tag', 'TaskControlGUI');
    ...
    % Listen to changes in the pause state
    addlistener(taskState, 'StateChanged', @updateGUIBasedOnTaskState);
