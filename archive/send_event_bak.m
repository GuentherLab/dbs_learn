function [event_times,code_sent,dio] = send_event(eventCode,dio,Interval,Dur,Rep,devID,start_delay,varargin)
    %% output
    % 1) "event_times": [N x 1]
    %% event code
    ...
    if nargin<7, start_delay = 0; end
    if nargin>=8, verbose = varargin{1}; else; verbose = default_verbose; end
    ...
    event_times = [];
    t = timer('StartFcn',@(~,~)fprintf('timer started, %1.0f beacons in %1.1f sec.\n',Rep,start_delay),...
